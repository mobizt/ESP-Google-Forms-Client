...
// This example shows how to create form watch.
...
void loop()
{
    ...
    if (ready && !taskComplete)
    {
        ...
        Serial.println("\nCreate a new form watch...");
        Serial.println("------------------------------");

        FirebaseJson request, response;

        // For Google Forms API ref doc, go to https://developers.google.com/forms/api/reference/rest/v1/forms.watches/create

        request.set("watch/target/topic/topicName", "<topicName>" /* topic name e.g. projects/your_project/topics/your_topic */);
        request.set("watch/eventType", "SCHEMA");
        ...
        // The watch using service account credentials is currently not working.
        ...
        bool success = GForms.createWatch(&response, "<formId>" /* form ID to request */, &request);
        if (success)
