...
// This example shows how to list a form's watch IDs.
...
void loop()
{
    ...
    if (ready && !taskComplete)
    {
        ...
        Serial.println("\nGet list of a form's watch ID...");
        Serial.println("------------------------------");

        FirebaseJson response;

        std::vector<String> watchIds;

        bool success = GForms.getWatchIDList("<formId>" /* form ID to request */, watchIds);
        if (success)
        {
            for (size_t i = 0; i < watchIds.size(); i++)
            {
                Serial.println(watchIds[i]);
            }
        }
        else
