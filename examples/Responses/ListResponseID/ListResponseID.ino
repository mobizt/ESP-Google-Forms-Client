...
// This example shows how to list a form's response IDs.
...
void loop()
{
    ...
    if (ready && !taskComplete)
    {
        ...
        Serial.println("\nGet list of response ID from a form...");
        Serial.println("------------------------------");

        std::vector<String> responseIds;
        bool success = GForms.getResponseIDList("<formId>" /* form ID to request */, responseIds);

        if (success)
        {
            for (size_t i = 0; i < responseIds.size(); i++)
            {
                Serial.println(responseIds[i]);
            }
        }
        else
