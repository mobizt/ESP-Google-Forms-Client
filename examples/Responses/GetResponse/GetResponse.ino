...
// This example shows how to get a form's response.
...
void loop()
{
    ...
    if (ready && !taskComplete)
    {
        ...
        Serial.println("\nGet a form's response...");
        Serial.println("------------------------------");

        FirebaseJson response;

        // For Google Forms API ref doc, go to https://developers.google.com/forms/api/reference/rest/v1/forms.responses/get

        bool success = GForms.getResponse(&response /* returned response */, "<formId>" /* form ID to request */, "<responseId>" /* response ID */);
        if (success)
